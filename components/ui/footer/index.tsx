  ...
  return (
    <div className='w-full flex justify-center items-center flex-col'>
      <div className='w-full max-w-screen-xl flex justify-between flex-col md:flex-row gap-8 md:gap-16 py-9 md:py-14 px-3 md:px-8 2xl:px-0'>
        <div className='w-full max-w-44 flex flex-col gap-3'>
          ...
          {data.footer.topLinks.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className=' text-sm font-medium text-fg-text-contrast hover:text-fg-text'
            >
              {item.name}
            </a>
          ))}
            <div className='w-fit flex items-center gap-3 text-primary-text-contrast'>
              {data.footer.SocialMediaLinks.map((link) => (
                <Link
                  ...
                  className='text-xl text-primary-solid hover:text-primary-solid-hover'
        ...
        <div className='flex w-fit gap-20 lg:justify-center'>
      ...
      <div className='w-full flex justify-center pt-3 pb-8 '>
        <div className='w-full max-w-screen-xl flex flex-col items-center gap-y-7 lg:gap-y-9 px-3 md:px-8 2xl:px-0'>
          <div className='w-full flex-col md:flex-row flex md:justify-between md:items-center gap-y-3'>
            {/* <div className='w-fit flex gap-4'>
              {data.navbar.links.map((item) => (
                <a key={item.name} href={item.href}>
                  <p
                    className={classNames(
                      'text-sm font-medium text-fg-text-contrast hover:text-primary-solid-hover '
                    )}
            ...
            <p className='hidden md:block text-sm text-fg-text-contrast cursor-pointer'>
              Realised by{' '}
              <a
